...
class RomsData:
    ...
    def __init__(self, filename=None, download=False):
        ...
        download : bool, optional
            Indicate whether to download and save the data as a netCDF
            file with the provided URL
        """
    ...
    def open(self, filename, download=False):
        ...
        download : bool
            Indicate whether to download and save the data as a netCDF file
        ...
        if download:
            if not os.path.isfile(filename):
                ...
                file_name = f"romsdata_{time_info}.nc"
    ...
    def get_grid_info(self):
        if self._filename is not None:
            ...
            try:
                ...
                for index, dim in enumerate(self._grid_list):
                    if len(dim) > 1:  # suppose dim is 2-3D (exclude time dim)
                        ...
                        grid_info[index] = {
                            "type": "rectilinear",
                            "shape": tuple(
                                [self.data.dims[dim_name] for dim_name in dim]
                            ),
                            "grid_spacing": (
                                dim_info[y_name]["spacing_y"],
                                dim_info[x_name]["spacing_x"],
                            )
                            if z_name is None
                            else (
                                dim_info[z_name]["spacing_z"],
                                dim_info[y_name]["spacing_y"],
                                dim_info[x_name]["spacing_x"],
                            ),
                            "grid_origin": (
                                dim_info[y_name]["origin_y"],
                                dim_info[x_name]["origin_x"],
                            )
                            if z_name is None
                            else (
                                dim_info[z_name]["origin_z"],
                                dim_info[y_name]["origin_y"],
                                dim_info[x_name]["origin_x"],
                            ),
                            "grid_x": dim_info[x_name]["grid_x"],
                            "grid_y": dim_info[y_name]["grid_y"],
                            "grid_z": dim_info[z_name]["grid_z"]
                ...
                raise DataError(
                    f"Failed to get the grid information for {dim} from the dataset."
                ) from exc
    ...
    def get_time_info(self):
        if self._filename is not None:
            try:
                # identify time dim name and suppose there is only one time
                # dimension used for all variables
                if self._time_list:
                ...
                raise DataError(
                    "Failed to get the time information from the dataset."
                ) from exc
            ...
            try:
                for var_name, grid_id in self._var_list:
                    ...
                    var_info[var.long_name] = {
                        ...
                        "nbytes": var.values[0].nbytes
                        if len(var.dims) >= 3
                        else var.values.nbytes,  # current time step nbytes
                ...
                raise DataError(
                    f"Failed to get the variable information for"
                    f" {var_name} from the dataset."
                ) from exc
            ...
            for var_name in self._data.data_vars.keys():
                ...
                if len(var_obj.shape) in [3, 4]:  # get var with 3-4 dims
                    ...
                    if dim_names == [
                        "eta",
                        "xi",
                    ]:  # var with 2 dims and defined with geolocation
                        var_list.append(var_name)
                        if var_dims not in grid_list:
                            grid_list.append(var_dims)
    ...
    def _get_dim_info(self):
        ...
        dim_list = list({item for sublist in self._grid_list for item in sublist})
        ...
        if dim_list:
            ...
            for dim_name in dim_list:
                if "s_" in dim_name:
                    grid_z = np.arange(
                        0, self._data.dims[dim_name], dtype=float
                    )  # value represent layer index number
                    dim_info[dim_name] = {
                        "grid_z": grid_z,
                        "origin_z": grid_z[0],
                        "spacing_z": 1.0,
                    ...
                    if dim_name.replace("xi", "x") in self._data.data_vars.keys():
                        ...
                        grid_x = np.arange(
                            0, self._data.data_vars[coor_var].shape[1], dtype=float
                        )  # index value
                        dim_info[dim_name] = {
                            "grid_x": grid_x,
                            "origin_x": grid_x[0],
                    ...
                    if dim_name.replace("eta", "y") in self._data.data_vars.keys():
                        ...
                        grid_y = np.arange(
                            0, self._data.data_vars[coor_var].shape[0], dtype=float
                        )  # index value
                        dim_info[dim_name] = {
                            "grid_y": grid_y,
                            "origin_y": grid_y[0],
